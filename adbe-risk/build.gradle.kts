// adbe-risk: the AI risk substrate (ADR 0012). An Edge / analytics bounded
// context that follows the domain event journal (ADR 0011, stream 108) via
// adbe-read's EventJournalFollower and turns the event stream into live risk
// signals - per-account transaction-velocity z-scores and money-flow graph
// centrality - surfaced on a Netty HTTP dashboard.
//
// Like adbe-read, this module is NOT the deterministic core: it may use the
...
application {
    mainClass.set("com.adbe.risk.RiskServiceLauncher")
...
dependencies {
    ...
    implementation(project(":adbe-read"))
