...
public final class ClusterConfig {
     ...
     *
     * <p>Recognised keys: {@code ledgerd.clusterMembers} (required, member format),
     * {@code ledgerd.baseDir} (directory root, default {@code build/ledgerd-node-<id>}),
     * and {@code ledgerd.host} (this node's host, default {@code localhost}).
     */
    public static ClusterConfig fromProperties(final Path propertiesFile, final int nodeId) {
        ...
        final String members = props.getProperty("ledgerd.clusterMembers");
        if (members == null || members.isBlank()) {
            throw new IllegalArgumentException("ledgerd.clusterMembers is required in " + propertiesFile);
        }
        final Path baseDir = Paths.get(props.getProperty("ledgerd.baseDir", "build/ledgerd-node-" + nodeId));
        final String host = props.getProperty("ledgerd.host", "localhost");
