...
import io.justrade.ledgerd.bench.store.DataStore;
import io.justrade.ledgerd.bench.store.LedgerdDataStore;
import io.justrade.ledgerd.bench.store.PostgresDataStore;
import io.justrade.ledgerd.bench.store.RedisDataStore;
 ...
 * Testcontainers (Docker required); LEDGERD boots an in-process three-node cluster
 * unless {@code LEDGERD_INGRESS_ENDPOINTS} points at an external one.
 *
 * <pre>{@code
 * ./gradlew :bench:run --args="--accounts=1000 --ops=50000 --warmup=10000"
 * }</pre>
 *
...
public final class BenchmarkHarness {
    ...
    public static void main(final String[] args) {
        ...
        System.out.printf(
                "LEDGERD datastore benchmark: accounts=%d warmup=%d ops=%d concurrency=%d mix(c/d/t)=%d/%d/%d seed=%d%n",
                config.accounts(),
                config.warmupOps(),
                config.measureOps(),
                config.concurrency(),
    ...
    private static DataStore create(final String backend, final BenchmarkConfig config) {
        return switch (backend) {
            case "ledgerd" -> new LedgerdDataStore(config.ledgerdEndpoints(), config.concurrency());
