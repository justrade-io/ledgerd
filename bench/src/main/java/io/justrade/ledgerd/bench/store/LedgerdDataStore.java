...
/**
 * LEDGERD backend: drives an Aeron cluster through {@link WriteClient}. By default it
 ...
 * exposes directly.
 */
public final class LedgerdDataStore implements DataStore {
    ...
    public LedgerdDataStore(final String externalEndpoints, final int maxInFlight) {
        ...
        final ClientConfig config = ClientConfig.builder(CLIENT_ID, ingressEndpoints)
                ...
                .build();
        this.client = new WriteClient(config, handler);
    }
    ...
    public void setup(final int accounts, final long initialBalance) {
        ...
        for (int i = 0; i < accounts; i++) {
            seed[i] = new Op(io.justrade.ledgerd.bench.OpType.CREDIT, i + 1L, 0L, initialBalance);
        }
        executeAll(seed);
    }
    ...
    private void executeAll(final Op[] ops) {
        ...
        while (client.pendingCount() > 0) {
            ...
            if (System.nanoTime() - deadline > 0) {
                throw new IllegalStateException("LEDGERD drain timed out with " + client.pendingCount() + " in flight");
            }
        }
    }
    ...
    private void submitWithBackpressure(final Op op) {
        ...
        while (true) {
            try {
                ...
                client.poll();
                if (System.nanoTime() - deadline > 0) {
                    throw new IllegalStateException("LEDGERD submit timed out (cluster not accepting commands)", e);
                }
            }
        }
    }
    ...
    public void verify(final long expectedSupply) {
        ...
        if (client.expired() != 0) {
            throw new IllegalStateException("LEDGERD expired " + client.expired() + " commands (dropped work)");
        }
        if (client.completed() != client.submitted()) {
            throw new IllegalStateException("LEDGERD completed " + client.completed() + " of " + client.submitted());
        }
    }
    ...
    private static Path createTempDir() {
        try {
            return Files.createTempDirectory("ledgerd-bench-cluster-");
