# Benchmark: ADBE vs PostgreSQL vs Redis
...
against ADBE, PostgreSQL, and Redis behind a common interface, and reports
...
| --- | --- | --- | --- |
| ADBE | 3-node Raft cluster (real consensus) | Replicated + Aeron Archive | Pipelined async, one client |
...
ADBE is doing strictly more work per op (three replicas, durable log) than the
...
Testcontainers; ADBE boots an in-process three-node cluster):

```bash
./gradlew :adbe-bench:run --args="--accounts=1000 --ops=20000 --warmup=5000 --concurrency=32"
```

Results are printed as a table and written to `adbe-bench/build/bench/results.csv`.
...
| `--concurrency` | 64 | ADBE `maxInFlight` and PG/Redis worker/connection count |
...
| `--backends` | adbe,postgres,redis | subset to run |
| `--csv` | build/bench/results.csv | output path |

Point ADBE at an external cluster (instead of the in-process one) by exporting
`ADBE_INGRESS_ENDPOINTS`, for example the docker-compose stack's
`0=localhost:20100,...`.
...
| --- | --- | --- | --- | --- | --- |
| adbe | 53527 | 522.8 | 3137.5 | 5345.3 | 5357.6 |
| postgres | 28277 | 705.0 | 3651.6 | 5832.7 | 9855.0 |
| redis | 25153 | 288.0 | 906.8 | 1739.8 | 3070.0 |

Reading these fairly: ADBE leads throughput while being the only replicated,
...
total supply equals the value derived from the (seeded) op stream, and ADBE asserts
...
- Not a CI gate - it needs Docker and is wall-clock sensitive. The hot-path JMH
  micro-benchmarks in `adbe-core` / `adbe-read` remain the gating benchmarks.
