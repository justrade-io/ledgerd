# 0007 - Standby-Only Read Side: Remove the Cluster-Follower Read Mode
...
**standby** read nodes (`StandbyReadNode`) that replicate state through the
...
(the follower) and `ADBE_MODE=standby` (the default). Maintaining both doubles
...
deployment in this repository (the compose topology runs standby nodes only).

Separately, the standby node only began following the live consensus log *after*
...
started cluster produced no snapshot recording, so a standby read node served
empty reads indefinitely. The standby read path was therefore not functional in
a running Docker deployment without an externally triggered snapshot.

## Decision

1. **Standardize on the standby read node.** Remove the cluster-follower read
...
2. **Follow the consensus log from position 0.** `StandbyReadNode` now starts
...
3. **Idiomatic Agrona agent.** The standby's single-writer loop runs as an
...
4. **Routable Archive call-back host.** The standby binds its Archive-facing
...
5. **Single Docker Compose topology.** `docker-compose.yml` now brings up the
   3-node write cluster plus one standby read node (`adbe-read-0`) on one
...
- **Positive**: The standby read path works on a fresh cluster with no snapshot
...
- **Negative**: A standby that starts late replays the consensus log from
...
- Multi-archive failover (the standby still connects to a single Archive
...
- Periodic snapshot scheduling on the write cluster (not required now that the
  standby follows from position 0; snapshots still occur on clean shutdown).
