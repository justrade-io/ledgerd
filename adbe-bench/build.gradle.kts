// adbe-bench: a public, reproducible benchmark harness that runs the same
// logical wallet workload (credit / debit / transfer over N accounts) against
// ADBE (an in-process 3-node Aeron cluster driven by AdbeClient), PostgreSQL
...
//
// Like adbe-examples, this module is illustrative infrastructure, not the
...
application {
    mainClass.set("com.adbe.bench.BenchmarkHarness")
...
// informative but non-fatal so the harness stays readable (matches adbe-examples).
